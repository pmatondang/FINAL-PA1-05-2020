<?php
    ...
    $query = " SELECT * FROM komentar WHERE parent_comment_id = '0' ORDER BY comment_id DESC";
    ...
    foreach($result as $row){
        $output .= '
            <div class="card card-default mb-2" style="color:black;">
    ...
    function get_reply_comment($connect, $parent_id = 0, $marginleft = 0){
        $query = "SELECT * FROM komentar WHERE parent_comment_id = '".$parent_id."'";
        $output = '';
        $statement = $connect->prepare($query);
        $statement->execute();
        $result = $statement->fetchAll();
        ...
        if($count > 0){
            foreach($result as $row){
                $output .= '
                    <div class="card card-default mb-2" style="margin-left:'.$marginleft.'px; color:black;">
                        <div class="card-header">By <b>'.$row["comment_sender_name"].'</b> on <i>'.$row["date"].'</i></div>
                        <div class="card-body">'.$row["comment"].'</div>
                        <div class="card-footer" align="right"><button type="button" class="btn btn-primary reply" id="'.$row["comment_id"].'">Reply</button></div>
                    </div>
